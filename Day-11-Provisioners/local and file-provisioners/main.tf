...
resource "aws_instance" "server" {
  ...
  connection {
    ...
    private_key = file("C:/Users/prade/.ssh/id_ed25519")  #private key path
    host        = self.public_ip
  }
 ...
 provisioner "local-exec" {
  ...
  provisioner "file" {
    source      = "file01"  # Replace with the path to your local file
    destination = "/home/ec2-user/file01"  # Replace with the path on the remote instance
  }
  # remote execution process 
  provisioner "remote-exec" {
    inline = [
"touch filegayatri",
"echo hello from aws >> filegayatri", ##mysql -h -u user -p
]
 }
}
